...
# To be run on a Kali Linux system for best results. --willc
...
if len(sys.argv) == 1:
    ...
    print "\nScan Options:"
    print "nikto for Apache enumeration"
    print "dirb to find directories"
    print "wpscan for WordPress sites"
    print "uniscan to prod ports for info"
...
if str(sys.argv[2]) == 'dirb' and str(sys.argv[3]) == 'full' and str(sys.argv[4]) == 'log':
    dirbfulllog="dirb %s /usr/share/dirbuster/wordlists/directory-list-1.0.txt -w -o %s" % (URL, IP)
